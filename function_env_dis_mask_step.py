...
class FunctionDisMaskStepEnv(gym.Env):
    def __init__(self, function, dim, bound, step_size = 0.1, max_steps_explore=100, reset_state=None, action_dim=1, failure_times_max1=1000, failure_times_max2=5, is_eval = False, eval_steps=100):
        ...
        self.bound = bound
        # 第一个维度的动作空间为0代表-1, 1代表+1, 第二个维度的动作空间为0到dim-1，表示选择哪个维度进行操作，第三个维度的动作空间为0代表1，1代表10, 2代表100
        self.action_space = gym.spaces.MultiDiscrete([2, self.action_dim, 10])  
