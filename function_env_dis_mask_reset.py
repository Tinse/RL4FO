'''
一个适用于单目标优化问题的强化学习环境，采用离散动作空间、掩码动作选择、随机初始状态跳出局部最优
'''
import gymnasium as gym
import numpy as np
from collections import deque

class FunctionDisMaskResetEnv(gym.Env):
    def __init__(self, function, dim, bound, step_size = 0.1, max_steps_explore=100, reset_state=None, action_dim=1, failure_times_max1=10000, failure_times_max2=1000, is_eval = False, eval_steps=100):
    ...
    def step(self, action):
        ...
        if self.explore_mode:  # 如果处于探索模式，则失败次数加1
            if (self.failure_times >= self.failure_times_max2) :  # 如果达到最大失败次数, 增加最大步数
                ...
                # 陷入局部最优解，且达到最大失败次数，无法跳出局部最优解
                self.reset_state = self.best + 0.1 * np.random.uniform(self.bound[0], self.bound[1], self.dim)  # 随机初始化状态
                print(f'!!!!!!!!!!! new reset state: {self.reset_state}')
