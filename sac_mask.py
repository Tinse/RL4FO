'''
使用stable-baselines3库中的SAC算法训练模型，解决function optimization问题
...
env = FunctionEnv(
    ...
    bound=[-10, 10],
    max_steps=1000,
    reset_state=np.array([-7.1]*4, dtype=np.float32)
)

# 创建SAC模型
model_name = "SAC_mask_sphere_step1_max1000_reward1"
checkpoint_cb = CheckpointCallback(save_freq=100_000, save_path='./logs/', name_prefix=model_name)


model = SAC("MlpPolicy", env, verbose=1, tensorboard_log=r'./tensorboard_logs/', device='cpu')
