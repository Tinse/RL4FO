...
pbounds = {}
for i in range(1, 13):
    pbounds[f'x{i}'] = (-1.5, 1.5)  # 修改参数范围为[-10, 10]
print(pbounds)

acquisition_function = acquisition.UpperConfidenceBound(kappa=0.01)  # 选择UCB作为采集函数
...
# optimizer.set_gp_params(alpha=1, n_restarts_optimizer=0)
...
# 添加标题和标签
plt.title('目标函数值优化过程', fontsize=16)
